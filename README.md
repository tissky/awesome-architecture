...
但有一件事正在发生,而且远比多数人愿意承认的更快:**「写代码」作为一种稀缺技能,正在终结。** 这不是危言耸听——在 OpenAI、Anthropic 这样的前沿实验室,绝大部分代码已经由 AI 写出,工程师早已不再逐行敲实现,而是退到上游:**决定让 AI 写什么,再判断它写得对不对。** 当机器几秒就能生成能跑的代码,"用 `for` 还是 `map`、记不记得某个 API、熟不熟某种语法"这种手艺,几乎一夜之间就不再是壁垒。
